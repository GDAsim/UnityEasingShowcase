Shader "SimpleRotateLerp"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline float4 QuatNLerp(float4 q1, float4 q2, float t)
            {
                // Dot product
                float dot1 = dot(q1, q2);

                // Ensure shortest path (avoid long way around sphere)
                if (dot1 < 0.0)
                {
                    q2 = -q2;
                    dot1 = -dot1;
            ...
            Varyings vert(Attributes IN)
            {
                ...
                float4 rot = QuatNLerp(_Rotation, _EndRotation,t);

                float4x4 model = BuildTRS(_Translation,rot,_Scale);
